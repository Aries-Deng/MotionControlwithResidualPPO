"""Part 3 - Unitree G1 dance tracking with PID + residual PPO.
...
Run from the repository root, for example:
  ...
  conda run --no-capture-output -n hw python -m hw_py.part3_dance_pid_residual_ppo

Outputs:
  - logs/rsl_rl/hw_part3_dance_pid_residual_g1/.../model_*.pt
  - hw_py/outputs/part3_dance_pid_residual_error.png
  - hw_py/outputs/part3_dance_pid_residual_video.mp4
  - hw_py/outputs/g1_pid_vs_pid_residual_tracking_error.png
"""
...
from hw_mjlab.train import latest_checkpoint
from hw_py.part3_dance_pid_baseline import (
  _make_pid_env_cfg,
  infer_action_scale,
  infer_num_actions,
...
EXPERIMENT_NAME = "hw_part3_dance_pid_residual_g1"
DEFAULT_CHECKPOINT = Path(__file__).parent / "part3_pid_residual_model_final.pt"
DEFAULT_BASE_CHECKPOINT = Path(__file__).parent / "part3_model_final.pt"
...
def _joint_tracking_error_tensor(env: ManagerBasedRlEnv) -> torch.Tensor:
  from hw_mjlab.dance import target_joint_pos
...
def _make_residual_env(
  ...
  base_rl_cfg = make_ppo_cfg("hw_part3_dance_single_g1", max_iterations=1)
...
def play(
  ...
  output_plot = out_dir / "part3_dance_pid_residual_error.png"
  plt.figure()
  plt.plot(metric_arr)
  ...
  output_video = out_dir / "part3_dance_pid_residual_video.mp4"
  if frames:
    media.write_video(str(output_video), frames, fps=int(round(1.0 / step_dt)))

  print(f"[Part 3 PID + residual PPO] Mean joint tracking error: {error:.3f} rad")
  print(f"[Part 3 PID + residual PPO] Plot: {output_plot}")
  print(f"[Part 3 PID + residual PPO] Video: {output_video}")
  return error
...
def compare_with_pid(
  ...
  plt.savefig(output_plot)
  plt.close()
  print(f"[Part 3 comparison] Plot: {output_plot}")
