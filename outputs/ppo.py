...
def monte_carlo_advantage(rewards: np.ndarray, values: np.ndarray, gamma: float):
    ...
    for t in range(len(rewards) - 1, -1, -1):
        ...
        advantages[t] = discounted_return - values[t]
...
def compute_policy_loss(ratio, adv, dist_entropy, epsilon, entropy_weight):
    ...
    unclipped_surrogate = ratio * adv
    clipped_ratio = np.clip(ratio, 1.0 - epsilon, 1.0 + epsilon)
    clipped_surrogate = clipped_ratio * adv
    surrogate = np.minimum(unclipped_surrogate, clipped_surrogate)

    return float(-np.mean(surrogate) - entropy_weight * dist_entropy)
