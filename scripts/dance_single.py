"""Single dance trajectory tracking (Unitree G1 humanoid).
...
from mjlab.tracking_rewards import (
...
def play(
  ...
  from mjlab.evaluate import evaluate
  ...
  error = evaluate(
    ...
    desired_value=0.0,
    num_steps=num_steps,
    output_plot=out_dir / "dance_single_error.png",
    output_video=out_dir / "dance_single_video.mp4",
    render_mode="rgb_array",
  )
  print(f"[dance play] Single-dance mean joint tracking error: {error:.3f} rad")
  return error


def main(num_envs: int = 4096, max_iterations: int = 2500) -> None:
  from mjlab.train import latest_checkpoint, train

  rl_cfg = make_ppo_cfg("hw_dance_single_g1", max_iterations=max_iterations)
