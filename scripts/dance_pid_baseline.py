"""Reference/PID-style dance tracking without PPO.

This script uses the same Unitree G1 robot, the same mjlab dance-tracking
environment, and the same hiphop reference trajectory as ``dance_single``.
...
from scripts.dance_single import _make_tracking_env_cfg
...
def play_pid(
  ...
  output_plot = out_dir / "dance_pid_error.png"
  plt.figure()
  plt.plot(metric_arr)
  ...
  output_video = out_dir / "dance_pid_video.mp4"
  if frames:
    fps = int(round(1.0 / step_dt))
    media.write_video(str(output_video), frames, fps=fps)

  print(f"[dance PID baseline] Mean joint tracking error: {error:.3f} rad")
  print(f"[dance PID baseline] Plot: {output_plot}")
  print(f"[dance PID baseline] Video: {output_video}")
  return error


def play_hybrid_pid(
  checkpoint_path: str | Path = Path(__file__).parent / "model_final.pt",
  output_dir: str | Path | None = None,
  num_steps: int = DEFAULT_TRACKING_STEPS,
  render_mode: str = "rgb_array",
) -> float:
  ...
  device = "cuda:0" if torch.cuda.is_available() else "cpu"
  env = ManagerBasedRlEnv(cfg=env_cfg, device=device, render_mode=render_mode)
  rl_cfg = make_ppo_cfg("hw_dance_single_g1", max_iterations=1)
  ...
  out_dir.mkdir(parents=True, exist_ok=True)
  output_plot = out_dir / "dance_hybrid_pid_error.png"
  plt.figure()
  plt.plot(metric_arr)
  plt.axhline(0.0, color="r", linestyle="--", label="desired")
  ...
  output_video = out_dir / "dance_hybrid_pid_video.mp4"
  if frames:
    media.write_video(str(output_video), frames, fps=int(round(1.0 / step_dt)))

  print(f"[dance hybrid PID] Mean joint tracking error: {error:.3f} rad")
  print(f"[dance hybrid PID] Plot: {output_plot}")
  print(f"[dance hybrid PID] Video: {output_video}")
  return error
